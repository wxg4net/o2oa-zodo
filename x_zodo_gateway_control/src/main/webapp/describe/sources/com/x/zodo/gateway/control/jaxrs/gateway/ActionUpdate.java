...
public class ActionUpdate extends BaseAction {
	...
	protected ActionResult<Wo> execute( HttpServletRequest request, EffectivePerson effectivePerson, String id, JsonElement jsonElement ) throws Exception {
		...
		try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
			ZodoGatewayLog zodoGateway = emc.find( id, ZodoGatewayLog.class );
			if( zodoGateway == null ){
				throw new ExceptionEntityNotExist(id, ZodoGatewayLog.class);
			}
			Wi.copier.copy(wi, zodoGateway);
			//启动事务
			emc.beginTransaction( ZodoGatewayLog.class );
			//校验对象
			emc.check( zodoGateway, CheckPersistType.all );
			//提交事务
			emc.commit();
