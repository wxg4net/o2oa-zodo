...
public class ActionListSubDirect extends BaseAction {

    private Logger logger = LoggerFactory.getLogger( ActionList.class );


    ActionResult<List<Wo>> execute(HttpServletRequest request, String distinguishedName) throws Exception {
        try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
            ActionResult<List<Wo>> result = new ActionResult<>();
            Business business = new Business(emc);
            Cache.CacheKey cacheKey = new Cache.CacheKey(this.getClass(), distinguishedName);
            Optional<?> optional = CacheManager.get(business.cache(), cacheKey);
            if (optional.isPresent()) {
                result.setData((List<Wo>) optional.get());
            } else {
                List<Wo> wos = this.list(business, distinguishedName);
    ...
    private List<Wo> list(Business business, String flag) throws Exception {
        ...
        for (Wo wo : wos) {
            ...
            wo.setLeaf(wo.getSubDirectUnitCount() == 0);
